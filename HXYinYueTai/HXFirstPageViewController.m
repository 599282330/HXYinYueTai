...
- (void) setHomePageControl
{
    ...
    [_homePageControl addTarget: self action: @selector(changeContent) forControlEvents: UIControlEventValueChanged];
}
...
- (void) refreshScrollView
{
    ...
    for (NSInteger index = 0; index < _suggestionModelArray.count ; ++index)
    {
        ...
        [imageView setImageWithURL: [NSURL URLWithString: suggestionModel.posterPicture] placeholderImage: [UIImage imageNamed: @"HomePage_Shadow"]];
    ...
    @synchronized(self)
    {
        ...
        _timer = [NSTimer scheduledTimerWithTimeInterval: 3.0f target: self selector: @selector(changeContent) userInfo: nil repeats: YES];
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    _timer = [NSTimer scheduledTimerWithTimeInterval: 3.0 target: self selector: @selector(changeContent)  userInfo: nil repeats: YES];
    [[NSRunLoop currentRunLoop] addTimer: _timer forMode: NSRunLoopCommonModes];
    
    NSInteger numberPage = _homePageScrollView.contentOffset.x / _homePageScrollView.frame.size.width;
    
    if (numberPage == 0)
    {
        _homePageControl.currentPage = _homePageControl.numberOfPages - 1;
        _homePageScrollView.contentOffset = CGPointMake(_homePageControl.numberOfPages * _homePageScrollView.frame.size.width, 0);
    }
    else if (numberPage == _homePageControl.numberOfPages + 1)
    {
        _homePageControl.currentPage = 0;
        _homePageScrollView.contentOffset = CGPointMake(1 * _homePageScrollView.frame.size.width, 0);
    }
    else
    {
        _homePageControl.currentPage = numberPage - 1;
    }
    ...
    [self adjustHeadViewWithIndex:index];
}

- (void)adjustHeadViewWithIndex:(NSInteger)index
{
    // 获取推荐模型
...
- (void) changeContent
{
    ...
    // 定时器切换页面
    NSInteger numberPage = _homePageScrollView.contentOffset.x / _homePageScrollView.frame.size.width;
    _homePageScrollView.contentOffset = CGPointMake((numberPage + 1) * _homePageScrollView.frame.size.width, 0);
    numberPage = _homePageScrollView.contentOffset.x / _homePageScrollView.frame.size.width;
    _homePageControl.currentPage++;
    if (numberPage == _homePageControl.numberOfPages + 1)
    {
        _homePageScrollView.contentOffset = CGPointMake(1 * _homePageScrollView.frame.size.width, 0);
        _homePageControl.currentPage = 0;
    }
